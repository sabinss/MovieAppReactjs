...
export const Detail = (props: any) => {
    ...
    const renderTvShowInfo = () => {
        return <div className="Movie_detail_info">
            ...
            {netwokrName()}
            {/* {
                (tvShowData?.networks && tvShowData?.networks.length > 0) && tvShowData?.networks[0].name.toLowerCase() == 'netflix' ?
                    <a href={tvShowData.homepage} style={{ "textDecoration": "none", "color": "white" }}>{tvShowData?.networks[0].name}
                    </a> : <p>{tvShowData?.networks[0].name}</p>
            } */}
    ...
    const netwokrName = () => {
        console.log(tvShowData?.networks)
        return '';
    }
    return (
        <>
            <div className="Wrapper">
                ...
                <div style={{ 'width': '100%', "display": 'flex', "justifyContent": 'end' }}>
                    <Button variant="link" onClick={() => { props.history.goBack() }} style={{ "textDecoration": 'none', "color": "white" }}>Go Back</Button>
                </div>
